import { useMutation, useQueryClient } from "@tanstack/react-query";
import { BreakdownActionRequest } from "../types/breakdown.type";
import { breakdownService } from "../service/breakdown.service";
import { QUERY_KEYS } from "@/constants";

interface UpdateBreakdownAction {
  id: number;
  actionId: number;
  payload: BreakdownActionRequest;
}

export const useUpdateBreakdownAction = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, actionId, payload }: UpdateBreakdownAction) =>
      breakdownService.updateBreakdownAction(id, actionId, payload),

    onSuccess: (_, variable) => {
      queryClient.invalidateQueries({
        queryKey: QUERY_KEYS.breakdown.list,
      });

      queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.breakdown.id, variable.id],
      });
    },
  });
};
